...
data = get_country_averages(data, ["new_cases_per_million", "new_vaccinations_smoothed_per_million"])


fig = make_subplots(specs=[[{"secondary_y": True}]])

fig.add_trace(
    go.Scatter(x=data['date'], y=data["new_cases_per_million30"], name="New deaths",line=dict(color='blue')),
    secondary_y=False,
)
...
fig.update_layout(hovermode="x", height=600, width=1000, template='plotly_white', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                  title = {'text': "New cases vs new vaccinations", 'y':0.9, 'x':0.5, 'xanchor': 'center', 'yanchor': 'top'},
