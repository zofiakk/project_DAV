...
Function which creates interactive line plots showing
the pandemic data in neighboring countries
"""
import pandas as pd
...
# Read teh data, choose its subset and calculate averages
...
for country, data_for_country in data.groupby("location"):
    fig.add_scatter(
        ...
        "Number of observed: %{y}<br>" + "<extra></extra>",
        line=dict(width=2),
        marker=dict(size=5, maxdisplayed=70),
        visible=True)

# Create dropdown menu and change displayed data
updatemenus = list([
    dict(active=0,
         yanchor="top",
         x=0.0,
